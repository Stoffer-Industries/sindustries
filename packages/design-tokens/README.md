...
packages/ui/component-catalog.json  ← React components per pack
     ...
     ├── design-systems.lib.pen  library copy of design-systems.pen (import target for main.pen)
...
That overwrites `styles.css`, `src/tokens.ts`, `pen-tokens.json`, and keeps `design-systems.pen` library-ready by removing nested imports, merging `themes` + `variables`, and refreshing its generated token specimen. The build then runs `sync:design-systems-lib` to copy the result into `design-systems.lib.pen` (the file `main.pen` imports). Commit generated files alongside token changes.
...
| `docs/designs/budgeting/main.pen` | `imports.ui` → `design-systems.lib.pen` | Components (`ui:…` refs) + `$si-color-*` / `$si-radius-*` on art |
...
- **`design-systems.lib.pen`** — flattened library copy of `design-systems.pen` and the file `main.pen` imports via `imports.ui`. Generated by `npm run build` (which runs `sync:design-systems-lib` after building `design-systems.pen`). Never hand-edit it.
- **`pen-tokens.json`** — same variable data as JSON for tooling (Pencil MCP `set_variables`, CI checks, etc.).
- Repo-root scripts:
  - `npm run sync:design-systems-lib` — copy `design-systems.pen` → `design-systems.lib.pen`. Runs automatically at the end of this package's build; run it standalone only if you edited `design-systems.pen` without rebuilding.
  - `npm run sync:budgeting-pen` — set `docs/designs/budgeting/main.pen` `imports.ui` → the library pen, strip duplicate root `variables`, remove legacy frame `q4Jkj` if present.
...
This validates `component-catalog.json` ↔ React exports, rebuilds, and checks `styles.css`, `src/tokens.ts`, `pen-tokens.json`, `design-systems.pen`, `design-systems.lib.pen`, and `packages/ui/src/specimen/generated/*`.
