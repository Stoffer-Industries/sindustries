...
| attentionOwners | `string[]` | Ordered action/escalation role slots. Index 0 is the next actionable owner; later entries are dormant escalation targets and repeated names are intentional. Quinn is the highest agent escalation. Tom at index 0 is terminal human action and needs no fallback; Tom later in the tail is dormant. Empty means delivery falls back to the assignee/workflow. |
