...
1. When `/workouts` finds no active OAuth consent, it shows **Connect to your agent** with explicit Claude and ChatGPT options, the remote MCP URL, and the seeded public client IDs.
   ...
   - **ChatGPT connector.** OpenAI's published docs confirm Developer Mode / custom MCP connectors with write actions are available on **any paid ChatGPT plan** — Plus, Pro, Business, Enterprise, or Edu — and only the Free tier is excluded ([OpenAI: Developer mode and MCP apps in ChatGPT](https://help.openai.com/en/articles/12584461-developer-mode-and-mcp-apps-in-chatgpt)). ChatGPT does not offer a pre-fillable deep link for the connector screen the way Anthropic does, so the card links to `https://chatgpt.com/` (the ChatGPT root) and walks the user through Settings → Apps → Create (Developer Mode), adding GymTrack with the GymTrack MCP URL, choosing OAuth, scanning tools, authorising, and saving. The seeded `chatgpt` OAuth client row in `public.gymtrack_oauth_clients` (registered redirect URI `https://chatgpt.com/connector_platform_oauth_redirect`, migration `20260804070000_mcp_oauth.sql`) is reused end-to-end — no new migration, no new env var, no new service. Card copy includes a Free-tier note so a Free-tier user is not led into a dead end. Re-added under task `696f2487-4817-4ecb-83fb-480b6ce654cc` (2026-09-10) after Tom verified on a paid-tier ChatGPT account that Developer Mode / custom MCP connectors with write actions are reachable on Plus-or-above plans. The prior task `91994011-5c34-4af5-8ef4-40daac604e64` (2026-08-23) removed ChatGPT on a now-superseded assumption that custom MCP connectors with write actions were Business / Enterprise / Edu only; that design is annotated as superseded by `docs/specs/gymtrack-readd-chatgpt-connect-option-2026-09-10-tech-design.md`.
...
6. `/agent-consent` shows the client name (with `client_id` fallback) and the requested scopes. The registered `redirect_uri` is intentionally not surfaced to the user — it is a server-side OAuth concern, and the security-relevant signal the user actually inspects is the requested scopes.
...
| `/workouts` | `WorkoutsTab` | Pending planned workouts, soonest-first, with Today/Overdue badges; users with no active consent also see the Claude and ChatGPT connector deep links (task `696f2487-4817-4ecb-83fb-480b6ce654cc` re-added ChatGPT after the prior task `91994011` removal). |
