...
| `lobster_route_specs.py` | Approval | review lobster | **WS3 pure routing (task 536e04fc).** Runs AHEAD of `request_spec_approval` to split items into feature / direct / ambiguous buckets (`readyPackages`, `blockedPackages`, `directCreateItems`, `triageEvents`); appends to `bookmark-triage-queue.json` for ambiguous items. No state mutations, no approval halt. |
| `lobster_request_spec_approval.py` | Approval | review lobster | **WS3 finalize + gate (task 536e04fc).** Runs AFTER `route_specs`: applies Phase 3 state mutations for `reviewed` / `monitoring` / `blocked` items and passes the routed sub-buckets through. The `approval: required` YAML flag halts the pipeline here so `request_topic_approval.py` can deliver the Telegram approval message — but only feature items reach this gate because direct and ambiguous items already left the pipeline upstream. |
| `lobster_create_tasks_from_direct.py` | Task | review lobster | **WS3 direct-create (task 536e04fc).** Sits BETWEEN `route_specs` and the approval-halt step. Reads `directCreateItems` from `route_specs.json` and creates Tasks API tasks with `taskType=code`/`taskType=research` so a code/research-only batch creates its tasks in one cron tick without needing an unrelated feature approval to resume the pipeline (AC3 reliability fix). Idempotent via `existing_by_marker` dedup. |
