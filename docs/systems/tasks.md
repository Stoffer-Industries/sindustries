...
**Mechanical-evidence gate (doing → acceptance, task 5e35dc25):** after the lobster's `verify_delivery` runs the AC text / evidence annotation checks above, it runs the structural evidence gate in `agents/workflows/feature-task/src/ac_parsing.rs:mechanical_evidence_failures`. For each AC's parsed evidence it verifies (a) `(testID: <test_file>)` cites a test file in the PR diff, (b) `(not tested: <reason>)` remains an explicit human exception, (c) `(pr: <file_path>)` cites a file in the PR diff (`#N` and URL siblings are accepted by structure), and (d) `(not code: <reason>)` always passes. Ordinary test-name citations are execution evidence for GitHub CI and Ash; the lobster does not rerun them against its local checkout. Failures land in the same `[feature-task-progress-checklist]` bucket as the AC text checks — the assignee sees one comment per sweep with all structural + text problems grouped. **Gate ordering:** structural checks run first; if any fail, the lobster short-circuits and emits a `[qa-agent-blocked]` comment so the next sweep re-runs against a fresh PR state. Ash's structured `qa_agent` approval remains the semantic/external-check verification. Task 5e35dc25 is the full design; the gate's file/path checks remain lobster-side while test execution is owned by CI/Ash.
