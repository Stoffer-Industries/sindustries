...
Structured workflow-gate row. Each row is `(taskId, type, owner, state, approvedAt, revokedAt, note)`. `state` is `pending`, `approved`, or `revoked`. A task may have one row per approval type (`spec`, `tech_design`, `qa_agent`, `accepted`). Approved rows satisfy their gate; pending / revoked / missing rows leave the gate outstanding. Rows are owned by the gate policy (see [Required approvals policy](#required-approvals-policy)) — clients do not pass `owner` for approvals; the server derives it from the credential and policy.

`pending` rows are created by `POST /tasks` itself, one per required approval type, in the same transaction as the task row (task `d9cd8a83`). Every required gate therefore has a queryable, identity-stable row from the moment the task exists — no required gate is ever absent. The Tasks API never persisted `pending` rows before `d9cd8a83`; the feature-task lobster previously materialised a `revoked` row as a proxy ("revoke-as-outstanding") and `POST /tasks` simply omitted approvals. The proxy was a behavioural overload — the gate predicate only reads `state === 'approved'`, so `revoked` and `pending` are both "not satisfied" — but it left the audit trail ambiguous between "actively taken back" and "never granted". `d9cd8a83` replaces the proxy with a first-class state and removes the lobster bootstrap. Gate predicates (`task_approval_granted` in `agents/workflows/feature-task/src/task_approvals.rs`, the `(task.approvals ?? []).filter(... === 'approved' && !a.revokedAt)` predicate in `_mapper.ts`) treat `pending` identically to a missing/revoked row — no observable change to any existing task's pass/fail outcome.
...
**Mechanical-evidence gate (doing → acceptance, post-merge, task 5e35dc25):** after the lobster's `verify_delivery` runs the AC text / evidence annotation checks above, it runs the mechanical-evidence gate in `agents/workflows/feature-task/src/ac_parsing.rs:mechanical_evidence_failures` (runner implementations live in `agents/workflows/feature-task/src/test_runners.rs`). This gate is the deterministic file-existence + cited-test pass/fail check — for each AC's parsed evidence it verifies (a) `(testID: <test_file>)` cites a file in the PR diff or a test name that runs green, (b) `(not tested: <file_path>)` cites a file in the PR diff, (c) `(pr: <file_path>)` cites a file in the PR diff (`#N` and URL siblings are accepted by structure), (d) `(not code: <reason>)` always passes. Failures land in the same `[feature-task-progress-checklist]` bucket as the AC text checks — the assignee sees one comment per sweep with all mechanical + structural + text problems grouped. **Gate ordering:** mechanical checks run first; if any fail, the lobster short-circuits and emits a `[qa-agent-blocked]` comment so the next sweep re-runs against a fresh PR state (the `qa_agent` row is always present as a `pending` row from task creation — task `d9cd8a83` — so the predicate check below sees the row from the moment the task exists; Ash only flips it to `approved` after mechanical verification, so the qa_agent gate is scoped to semantic judgment (see `agents/ash/src/verify.ts:verifySemantic` — Quinn owns the real judgment implementation). The structural approval gate replaces what used to be Ash's mechanical checks (cited-file existence, cited-test pass/fail, evidence-text matching) — that surface is now lobsterside, so the assignee gets immediate deterministic feedback without waiting on Ash's cron. Task 5e35dc25 is the full design; PR #1 (ac_parsing additions) and PR #2 (verify_delivery wiring) are merged on main. This note was added when PR #3 (Ash-side slim) landed.
...
**Clippy-evidence gate (W36 audit A2, task afe39f3d):** an opt-in merge gate (`CLIPPY_ENFORCE=true`, default disabled until the clippy CI gate has been green for ≥1 week per task `55c98158`) checks every PR that touches `agents/workflows/feature-task/**` for the canonical `cargo clippy` command in its PR body. Implementation lives in `clippy_evidence_failures` (`agents/workflows/feature-task/src/pr_gates.rs`). **Fail-closed on `gh` errors (A2):** the file-list query (`pr_changed_files`) returns `Result<Vec<String>, String>` and a `gh` failure surfaces as a recorded `[feature-task-progress-checklist]` failure with the exit code and stderr — matching the existing `pr_body` Err discipline in the same function. The mechanical-evidence dispatch site (`verify_delivery`) additionally defaults `is_rust_pr: true` on `Err` so bare-Rust citations are over-checked (cargo runner) rather than under-checked (pnpm runner) during a transient `gh` blip. Empty-diff PRs (`Ok(vec![])`) preserve the existing label-only-PR contract — they do not touch the Rust workflow and do not fail the gate.
...
- `agents/workflows/feature-task/src/analytics.rs` — analytics emission and terminal summaries
- `agents/workflows/feature-task/src/analytics_replay.rs` — analytics replay subcommand
- `agents/workflows/feature-task/src/main.rs` — CLI dispatch wiring
...
| `FEATURE_TASK_LOBSTER_TOKEN` | `feature_task_lobster` | `agents/workflows/feature-task/src/api_client.rs` (`add_comment`), `agents/workflows/feature-task/src/analytics.rs` |
...
The lobster's approval-marker parsing and reconciliation live in `agents/workflows/feature-task/src/product_spec_parsing.rs` and `agents/workflows/feature-task/src/brain_spec_reconcile.rs`. It recognises three states for the `**Approved by Tom**` marker in the task description:
...
1. **Done transition (primary trigger).** Every code path that takes a feature task from `open`/`doing`/`acceptance` to `done` invokes the archive helper. Today that is `post-merge` (`agents/workflows/feature-task/src/post_merge.rs` → `agents/workflows/feature-task/src/brain_spec_lifecycle.rs:archive_done_task_spec`); the helper is idempotent and rewrites the task `**Spec:**` line in the same pass.
...
Lives in:
- `agents/workflows/feature-task/src/brain_spec_reconcile.rs` — `archive_task_spec_for_done_task` pure archive decision
- `agents/workflows/feature-task/src/brain_spec_lifecycle.rs` — `apply_archive_outcome` orchestration and `archive_done_task_specs_sweep` reconciliation sweep
- `agents/workflows/feature-task/src/product_spec_parsing.rs` — `parse_product_spec_ref` and `extract_spec_path_from_line` tolerant parser
...
- `agents/workflows/feature-task/src/brain_spec_lifecycle.rs` — `block_on_spec_drift_fluid`, safe brain spec rewrite, checksum reset, and resync orchestration
- `agents/workflows/feature-task/src/product_spec_parsing.rs` — approval marker helpers and fingerprint-bound `[spec-resynced]` handling
...
| `agents/workflows/feature-task/src/main.rs` | Rust CLI dispatch and shared crate types; gate, reconciliation, archive, and resync behavior lives in focused sibling modules |
