...
- **C3 — Dead one-shot migration script left in `prisma/`.** *(Fact, Low.)* `services/tasks-api/prisma/migrate-todo-to-ready.ts` is a January-era data migration. It’s not in the Prisma migration history and will fail (`status='todo'` no longer exists). Should move to `tools/` or be deleted. ✅ [PR #TBD](https://github.com/Stoffer-Industries/sindustries/pull/TBD)
