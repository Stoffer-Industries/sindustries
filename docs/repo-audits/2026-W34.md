...
- **Severity:** High. → Classification: `tracked-code-task` (auth-boundary reconciliation; small tech design recommended — see Open Questions OQ1). `➡️ Tracked by task afe1056c (afe1056c-1d7f-4600-9791-95acabfb2781)`.
...
- **Severity:** Medium. → Classification: `tracked-code-task` (pin the validated IP: resolve once, connect to the IP with `Host`/SNI preserved, or use an httpx transport with a pinned resolver). `➡️ Tracked by task d6198e83 (d6198e83-14d5-48de-9a7e-addfb65dec30)`.

### Testing

**[Medium] The entire `cto-craft-tweet-drafts` test suite — including the SSRF and durability tests — does not run in CI.** ✅ [PR #463](https://github.com/Stoffer-Industries/sindustries/pull/463)
