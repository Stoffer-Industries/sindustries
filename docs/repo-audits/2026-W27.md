...
| 0-B | Add precommit / CI check that rejects duplicate migration prefixes | `scripts/check-migration-prefixes.sh` (new), `Makefile`, `ci.yml` | Two migrations with identical 14-char prefix fail the check; doc in `services/tasks-api/README.md` | S | Low | ✅ [PR #140](https://github.com/Stoffer-Industries/sindustries/pull/140)
...
| 0-D | Add a vitest spec for `budget-api` that calls every route without a Bearer and expects 401 *(was W26 0-C, still open)* | `services/budget-api/test/auth-contract.test.ts` (new) | Documents the gap today; after Theme 1 lands, the spec flips from "documents gap" to "enforces rule". Path-parameter IDOR routes covered by PR #144; userId-from-body/query follow-up routes (six non-Akahu) covered by PR #155. Three Akahu routes (/akahu/sync, /akahu/exchange, /akahu/authorize-url) still pending — need akahuClient mocks + env-var setup | S | Low | ✅ [PR #144](https://github.com/Stoffer-Industries/sindustries/pull/144), [PR #155](https://github.com/Stoffer-Industries/sindustries/pull/155)
