...
| 0-C | Add `target/` and `**/target/` to root `.gitignore` | `.gitignore` | `git check-ignore agents/workflows/feature-task/target` returns `target/` from the tracked `.gitignore`, not `.git/info/exclude` | S | Low |
