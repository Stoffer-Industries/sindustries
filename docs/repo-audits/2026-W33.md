...
- **T0.1 — Backfill rotation/revocation tests for gymtrack-mcp.** Cover refresh-token rotation happy path, replay of a rotated token (must fail), and revoked-consent access (must fail). *Files:* `services/gymtrack-mcp/src/repo.js`, new test files. *AC:* tests assert a rotated refresh token cannot be reused and a revoked consent's access token is rejected. *Effort:* M. *Risk:* Low. *Deps:* QW1. ✅ [PR #TBD](https://github.com/Stoffer-Industries/sindustries/pull/TBD)
