...
3. Post-merge: Rowan observes next 3 consecutive heartbeat cycles at 07:00/07:30/08:00 NZST and confirms `agent_task_queue.py --assignee Rowan --json` completes within the heartbeat-slot watchdog (≤30s wall time) on the production workload (≥18 linked URLs across ≥27 active tasks). Steady-state target is ~17s — see AC6 for the revised measured SLO.
...
- **AC6 (revised 2026-09-09, Quinn decision):** `agent_task_queue.py --assignee <agent> --json` completes within the heartbeat-slot watchdog (≤30s wall time) on the production workload (≥18 linked URLs across ≥27 active tasks) for at least 3 consecutive heartbeat cycles. Steady-state observed on 2026-09-08 NZST: ~17s (17.32–17.64s across 3 consecutive `--verbose` runs on Rowan's workload). Per-call latency is bounded by `_gh_api(timeout=10)` and graceful `GhApiTimeout` handling, so the script never aborts on a single slow call. The original `<15s` target was a spec-time guess against a smaller baseline (12 linked URLs / 26 active tasks) and is dropped in favour of two complementary measures: the hard ceiling (≤30s) that maps to the heartbeat-slot watchdog and the measured steady-state (~17s) that maps to the observed production workload. Any future regression to >20s steady-state should reopen this task or spawn a follow-up to optimise further (e.g. reduce per-PR fetch work via batched GraphQL queries, cached check-run lookups).