...
- Update `agents/definitions/ash/{HEARTBEAT.md, WORKFLOW.md, SOUL.md}` so the reasoning-and-deferral pattern is canonical, and update the feature-task state machine so a linked capability dependency suppresses stale Quinn re-escalation. The `DoD.md` covers the structured-approval + routing semantics; the state-machine change keeps those semantics durable across lobster sweeps.
...
> 7. On a `deferred` AC caused by a missing Ash/codebase capability: post `[qa-agent-deferred] AC<N>: <reason>` and a matching capability request, but do not post the structured `qa_agent` approval. Quinn approves and creates a capability-extension task; the original task depends on that task, and the extension task has no dependency back. Ash reruns the original after the extension completes. A real delivery-evidence failure remains `[qa-agent-blocked]` and routes back to the delivery assignee.
...
| `[qa-agent-deferred]` | One or more ACs require a capability Ash lacks | no approval; Quinn creates an extension task, the original depends on it, and Ash reruns the original after it completes |

The lobster treats a deferred capability gap as a temporary Quinn handoff until the dependency edge to the capability-extension task exists. Once that edge is present, it must not re-add Quinn or emit a stale `[openclaw-needed]` handoff; the original task is dependency-blocked and the extension task is the actionable work item. Comments remain evidence/audit, while the structured approval row remains the QA gate.
...
- **Risk: capability gap explosion (every task has at least one deferred AC).** If Ash starts deferring routinely, the gate becomes meaningless.
  Mitigation: the two-strike rule (only create a follow-up task on the second deferral of the same gap) keeps the work queue clean. Quinn reviews the deferred report, creates the capability-extension task, and links the original task to it. Tom reviews the eventual fresh QA result at his `accepted` pass. The structured `qa_agent` approval remains withheld until every AC is rerun after the capability is available.
...
- **Q2.** Should the `[qa-agent-deferred]` convention also be a lobster-recognised gate signal (e.g. a third state between `verified` and `blocked`)? *Recommendation:* no — the lobster's gate remains binary (approved or not). The deferred comment is evidence for the capability-extension handoff; the dependency edge is the machine-readable wait state. Adding a tri-state approval would blur the distinction between an incomplete capability and an accepted deliverable.
...
| AC4 (Capability gap → deferred report with reason + spec link, dependency wait) | HEARTBEAT step 3 clause 7 + DoD.md describe the `[qa-agent-deferred]` convention. The original task depends on the capability-extension task, which remains actionable; after completion Ash reruns the original and only then posts approval. |
